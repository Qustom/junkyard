...
**Current milestone:** M1.12 (Scaling Debt Paydown — pre-M2 architecture cleanup; Director-directed 2026-07-10, opened ahead of M1.11's UG2/UG3) — **design LOCKED 2026-07-10 (D-RAT-1…7; V3b pursuer-migration design pending). Implements the M1.11 systems-state report's R2–R10 (R1/CSV deferred). Master contract: behavior-preserving (4 control layout fps byte-identical + full suite green); measure = the debt ledger (net LOC removed). **Waves 1–2 DONE + integrated on `main`@`11627d4` (7/7; Wave-1 0 dev, Wave-2 1 pre-ratified dev D-RAT-7); ▶ Wave 3 (V4 GameState split) dispatching. V3b design locked (1 Director call pending at Wave 4: NDR-V3b-1).** *(M1.11 build+UG1 published `m1-20260708-69446d5` — Director playtest → UG2 → UG3 pending, non-blocking. M1.10 TG2/TG3, M1.9 SG2/SG3, M1.7/M1.6 RG2/RG3 Director-pending; M1.8 CLOSED.)*
...
## ▶ Next action (start here on a cold restart) — **M1.12 Wave 3** (V4 — GameState split, solo worktree, highest-contention file) dispatching; design-LOCKED (facade-preserving, D-RAT-4). Then Wave 4 = V3→V3b (V3b needs the NDR-V3b-1 `opposition_credits` Director call before it dispatches), Wave 5 = VG1→VG2→VG3. After each wave lands+integrates on `main`: run the close-out deviation sweep (Director dispositions), then dispatch the next. Master contract every wave: 4 control layout fps (`e943ac9c8bc1` + band_greybox/two/three) byte-identical + full suite green (V4 adds: meta save bytes byte-identical v4). Breakdown + D-RAT-1…7: `design/M1_12_Tasks/M1.12_Breakdown.md`; queue: `TASKS.md` §M1.12; board ids: `design/M1_12_Tasks/.board_item_ids.txt`.
>
