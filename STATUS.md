...
## ▶ Next action (start here on a cold restart) — **M1.4 Wave 3: K5a ∥ K5b ∥ K5c → K5i (hazard variety)**

Waves 1 & 2 done. **Wave 3 = new hazard-type variety** (parallel-then-integrate): **K5a ∥ K5b ∥ K5c** (three new hazard types, file-disjoint, `isolation: worktree`) → **K5i** (integration/wiring; `blockedBy: K5a,K5b,K5c`). Read `design/M1_4_Tasks/M1.4_Breakdown.md` Wave-3 build order + each `K5*_*.md` design for the collision plan (who writes `main_game.gd`/spawn wiring — likely K5i owns the integration seam, K5a/b/c self-contained hazard scripts). Then **Wave 4** = re-gate (RG1 build+verify → Director playtest → RG2 analysis → RG3 verdict). Standing contracts unchanged (all-off fp e943ac9c8bc1; preset ships fun values; single-writer-per-`.gd`; parallel agents `isolation: worktree`; verify branch topology before every merge; push + board mirror after each merge; wave close-out sweep).
