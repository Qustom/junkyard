...
> **Wave-3 close-out (awaiting Director):** 1 deviation **W3-F1** in `DESIGN_DEVIATIONS.md` (K5b test `queue_free`-on-freed stderr noise; test passes; rec **Addressed** — guard with `is_instance_valid`). Deferred Director taste/sweep calls (no action now, RG1 sweeps them): `NEW_HAZARD_BAND_CEILING=48` value; OQ-4 pure-deterministic vs local-sub-stream striping; K7 preset `exit_keep_one_at_spawn`/`exit_enabled` (ship OFF).
