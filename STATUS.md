...
## ▶ Next action (start here on a cold restart) — **M1.3 Wave-2 CLOSE-OUT deviation sweep (Director dispositions), then Wave 3 RG1**

Wave 2 (J2+J3+J4) is on `main`, verified, pushed, board=Done; all-off fp `e943ac9c8bc1` byte-identical; 46/46 knobs; schema v1 intact. **Before dispatching Wave 3, run the wave close-out deviation sweep** (`CLAUDE.md` → "Wave close-out"): assemble every `design/DESIGN_DEVIATIONS.md` entry + the queued Director-review observations, attach a recommendation, and the **Director dispositions each** (Reviewed/Addressed) — Claude does NOT self-disposition. Queued for the sweep:
1. **J2 curve-bias** — `curve` mode uses `pow(t,1.6)` which biases *shallower* not deeper (label says "deeper-biased"). Harmless: `curve` is preset-OFF (preset = `even_spread`). Rec: **Reviewed** (leave; if `curve` swept on at RG2, flip direction + fix comment).
2. **J3 Q-F wake-cadence observation** — density hazards seeded in deep big rooms may wake immediately (R1 depth-gate) → deep rooms could read instant-death. Not a J3 change; an **R1 `r1_depth_threshold`/`r1_linger_seconds` preset tuning** call for RG1. Rec: **Reviewed**, tune at RG1 playtest.
3. **(carryover, fun calls for the preset sweep)** J2 preset count/min-depth, J3 density/cap/metric, J4 `lvl_corridor_weight_mult`/`lvl_short_corridors` preset values — all Director RG2 sweeps, not fixes.

Then **Wave 3 — RG1** (qa-playtest-coordinator + general-purpose): author the M1.3 RG1 build+verify doc from the `design/M1_1_Tasks/RG1_playtest_build.md` template; assemble + verify the M1.3 loop (default preset boots, every knob fires, config-marked telemetry incl. the new `corridor_summary` row clean); **publish the build to itch** via `tools/push_itch.sh` (the standing playtest-gate step; **human-gated network** — SETUP §1a, Chromium-only). Then **RG2/RG3 are HUMAN-GATED**: Director playtests, qa analyses telemetry vs M1.0/M1.1/M1.2 baselines (RG2), Director records go/iterate/pivot in `design/M1_3_Tasks/G4_findings_M1.3.md` (RG3).
