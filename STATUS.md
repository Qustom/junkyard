...
## ▶ Next action (start here on a cold restart) — **BUILD PHASE COMPLETE (V1–V9 + V3b).** Next is **Wave 5 regression gate**: VG1 (full regression matrix + M1.11→M1.12 changelog + itch publish) → VG2 (equivalence/regression analysis) → VG3 (Director verdict → M2). **⚠ 2026-07-11 Director directive: STOP all CI/CD (GitHub LFS bandwidth limit) — nightly push disabled.** This affects VG1's itch-publish step — surface to the Director whether to still publish (local butler push doesn't burn GitHub LFS bandwidth, but the directive signals reducing publish activity) or run VG1 verify+changelog only. Master contract holds: 4 control layout fps byte-identical + full suite green. Breakdown + D-RAT-1…8: `design/M1_12_Tasks/M1.12_Breakdown.md`; queue: `TASKS.md` §M1.12; board: `design/M1_12_Tasks/.board_item_ids.txt`.
>
