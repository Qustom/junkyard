...
**Current milestone:** M1.12 (Scaling Debt Paydown — pre-M2 architecture cleanup; Director-directed 2026-07-10, opened ahead of M1.11's UG2/UG3) — **design LOCKED 2026-07-10 (D-RAT-1…7; V3b pursuer-migration design pending). Implements the M1.11 systems-state report's R2–R10 (R1/CSV deferred). Master contract: behavior-preserving (4 control layout fps byte-identical + full suite green); measure = the debt ledger (net LOC removed). **Waves 1–3 DONE + integrated on `main`@`0ed646f` (8/8; deviations: Wave-2 1 pre-ratified D-RAT-7, Wave-3 1 rec-Reviewed V4-quota-delegate); ▶ Wave 4 (V3→V3b) — GATED on 2 Director calls first: NDR-V3b-1 (`opposition_credits` number) + the V4 deviation disposition.** *(M1.11 build+UG1 published `m1-20260708-69446d5` — Director playtest → UG2 → UG3 pending, non-blocking. M1.10 TG2/TG3, M1.9 SG2/SG3, M1.7/M1.6 RG2/RG3 Director-pending; M1.8 CLOSED.)*
...
## ▶ Next action (start here on a cold restart) — **M1.12 Wave 4 (V3→V3b) is GATED on 2 Director calls** surfaced at this boundary: (1) **NDR-V3b-1** — the single `BandProfile.opposition_credits` number for band_greybox (rec ~55–65 to cover K5+pursuer, reading D-RAT-3b's "~48" as "preserve density"; ±15%-per-type and a hard-48 can't both hold since the pursuer was always additive); (2) the **V4 private-quota-delegate deviation** (rec Reviewed). Once dispositioned: dispatch V3 (K5 lane→deck, authors `opposition_credits`) → then V3b (pursuer→deck) sequentially in Wave 4, each with golden-capture-BEFORE-delete + equivalence re-pin. Then Wave 5 = VG1→VG2→VG3 (regression gate). Master contract every wave: 4 control layout fps byte-identical + full suite green; V3/V3b add the greybox hazard-spawn equivalence proof (±15% per-type, layout fps still byte-identical). Breakdown + D-RAT-1…7: `design/M1_12_Tasks/M1.12_Breakdown.md`; queue: `TASKS.md` §M1.12; board: `design/M1_12_Tasks/.board_item_ids.txt`.
>
