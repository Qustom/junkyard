...
## ▶ Next action (start here on a cold restart) — **M1.3 Wave 3: RG1 IN PROGRESS → then human-gated playtest (RG2/RG3)**

Wave 2 done + close-out swept (1 deviation Director-Reviewed, archived; `DESIGN_DEVIATIONS.md` empty). **RG1 dispatched** (`qa-playtest-coordinator`, worktree): authoring `design/M1_3_Tasks/RG1_playtest_build.md` (from the M1.2 RG1 template) + `tests/test_rg1_m13_verify` headless driver. Verifies: default preset boots; J2 spread / J3 density / J4 corridor-lever each take effect from CFG; the new `corridor_summary` JSONL row writes clean (corridor_s/room_s/corridor_frac); all-off baseline byte-identical (fp `e943ac9c8bc1`); 46 knobs in the `run_started` snapshot; schema v1 + `run_ended` arity intact.

When RG1 returns: verify topology → merge → push → board=Done → mini close-out. Then:
- **itch publish** via `bash tools/push_itch.sh` (`BUTLER=/mnt/c/wsl-libraries/butler/butler`) — **human-gated network** (sandbox can't reach `broth.itch.ovh`; SETUP §1a; Chromium-only). Standing playtest-gate step.
- **RG2/RG3 HUMAN-GATED:** Director playtests the swept configs (incl. the J3 wake-cadence tuning — `r1_depth_threshold`/`r1_linger_seconds` so deep big rooms aren't instant-death — and the mult-40 "void feel" window pass); `qa` analyses telemetry vs M1.0/M1.1/M1.2 baselines (RG2); Director records go/iterate/pivot in `design/M1_3_Tasks/G4_findings_M1.3.md` (RG3).
