...
**Current milestone:** M1.12 (Scaling Debt Paydown — pre-M2 architecture cleanup; Director-directed 2026-07-10, opened ahead of M1.11's UG2/UG3) — **design LOCKED 2026-07-10 (D-RAT-1…7; V3b pursuer-migration design pending). Implements the M1.11 systems-state report's R2–R10 (R1/CSV deferred). Master contract: behavior-preserving (4 control layout fps byte-identical + full suite green); measure = the debt ledger (net LOC removed). **BUILD PHASE COMPLETE — Waves 1–4 (V1–V9 + V3b) DONE + integrated on `main`@`c57ebf2`; DR-3 signed off; all 5 deviations dispositioned + archived. ▶ Wave 5 (VG1 regression build + itch publish → VG2 → VG3 verdict).** "Exactly one way to add an opposition" fully delivered (all 3 greybox spawn machines → deck lane); net debt removed across R2–R10. *(M1.11 build+UG1 published `m1-20260708-69446d5` — Director playtest → UG2 → UG3 pending, non-blocking. M1.10 TG2/TG3, M1.9 SG2/SG3, M1.7/M1.6 RG2/RG3 Director-pending; M1.8 CLOSED.)*
