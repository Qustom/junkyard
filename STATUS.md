...
**Current milestone:** M1.12 (Scaling Debt Paydown — pre-M2 architecture cleanup; Director-directed 2026-07-10, opened ahead of M1.11's UG2/UG3) — **design LOCKED 2026-07-10 (D-RAT-1…7; V3b pursuer-migration design pending). Implements the M1.11 systems-state report's R2–R10 (R1/CSV deferred). Master contract: behavior-preserving (4 control layout fps byte-identical + full suite green); measure = the debt ledger (net LOC removed). **✅ M1.12 CLOSED 2026-07-12 — Director verdict GO → M2.** Full R2–R10 scaling-debt paydown shipped behavior-preserving: build phase (V1–V9 + V3b) + VG1-fix + regression gate (VG1/VG2/VG3) all done + integrated on `main`. Game provably unchanged (4 control fps byte-identical, suite green); net-negative-LOC debt ledger; DR-3 signed off; 6 deviations dispositioned + archived. Verdict + evidence: `design/M1_12_Tasks/G4_findings_M1.12.md`. Build NOT published (CI/CD stopped for LFS bandwidth; verify+changelog only). **▶ Next milestone: M2** (crafting/upgrades/instability) — awaiting Director direction to open its breakdown; deferred **R1 (CSV item catalog)** now unblocked as a natural next content iteration. *(M1.11 build+UG1 published `m1-20260708-69446d5` — Director playtest → UG2 → UG3 pending, non-blocking. M1.10 TG2/TG3, M1.9 SG2/SG3, M1.7/M1.6 RG2/RG3 Director-pending; M1.8 CLOSED.)*
...
## M1.12 Wave 2 — Done + integrated on `main` (2026-07-10)
| Task | Agent | Merged | Proof |
|---|---|---|---|
| V2 — retire dual-emit signals (R3) | general-purpose | `feat/V2-retire-dual-emit` `b6f0314` | EventBus **60→54**; `_emit_family` (10 hosts) gone; 3 telemetry consumers re-pointed to `opposition_event`; 5 golden traces changed emit-column ONLY (positions byte-identical); parity fp `e943ac9c8bc1` equal; no `SCHEMA_VERSION` bump. 1 deviation (D-RAT-7 payload drop, pre-ratified). |
| V6 — RNG.substream helpers (R7) | general-purpose | `feat/V6-rng-substream` `a6d8086` | `substream` (XOR/.seed) + `substream_hashed` (boost/.seed+.state); 5 sites migrated byte-equal (new mandatory pockets golden = sole guard); `_stage_seed`/`_mix`/`_substream_seed` dedup'd; all 4 control fps byte-identical; 0 deviations. |

> **Integrated verify ALL GREEN (`main`@`11627d4`):** import · **parity fp `e943ac9c8bc1`** · band_two · band_three · bandgen_determinism · `test_rng_substream` (5 goldens) · band_flavors · band_depth (junk fp) · telemetry_jsonl · telemetry_config_marking · opposition_components (S2 golden parity, 5 traces) · def_menu_coverage (bijection) · smoke. **Wave-2 close-out: 1 deviation, pre-dispositioned via D-RAT-7 (no new Director action).** Minor hygiene follow-up: `test_def_menu_coverage`'s success string still says "dual-emit intact" (stale text; test passes) — fold into a later hygiene pass.

## M1.12 Wave 3 — Done + integrated on `main` (2026-07-10)
| Task | Agent | Merged | Proof |
|---|---|---|---|
| V4 — split GameState (R5) | general-purpose | `feat/V4-split-gamestate` `f8a2adf` | `game_state.gd` 752→**467** + `economy.gd` 251 + `quota_ladder.gd` 96; facade getter+setter → **zero caller edits** (38 callers); meta bytes byte-identical (sha256 match, 368 B, v4); v1/v2/v3→v4 green; 4 control fps byte-identical; autoloads still 6; quota 4 snapshots→1 held ref. 1 dev (private `_evaluate_quota` delegate, rec Reviewed). |

> **Integrated verify ALL GREEN (`main`@`0ed646f`):** import (0 script errors) · parity fp `e943ac9c8bc1` · save_migration v1/v2/v3→v4 · shop_economy · money_ledger · quota_system · loop_drive · main_game_loop · smoke. **Wave-3 close-out: 1 deviation (rec Reviewed) — batched to the Wave-4 Director boundary.**

## M1.12 Wave 4 — Done + integrated on `main` (2026-07-11); DR-3 signed off
| Task | Agent | Merged | Proof |
|---|---|---|---|
| V4b — address quota-delegate deviation | general-purpose | `feat/V4b-...` `6e36e57` | rewrote 6 `test_quota_system` sites onto `gs.quota_ladder().evaluate(...)`; private delegate dropped; quota/save/parity green. |
| V3 — K5 lane→deck (R4.1) | general-purpose | `feat/V3-k5-migration` `6e6c956` | `opposition_credits=48`, `per_band_cap 16`/def; **K5 equiv EXACT 0.0%Δ**; config 91→70; 4 layout fps byte-identical; net −201 LOC. 2 dev (Reviewed). |
| V3b — R1 pursuer→deck (R4.2) | general-purpose | `feat/V3b-pursuer-migration` `f4dddff` | `opposition_credits`→**58**; **pursuer equiv ±15%** (+11.1% deep = licensed J3-fold); `trace_pursuer_room/chase` byte-identical; config 70→**52**; net −395 LOC. 1 dev (Reviewed). |

> **Integrated verify ALL GREEN (`main`@`c57ebf2`):** import · parity fp `e943ac9c8bc1` · band_two · band_three · config 52/52 · V3 EQUIV (K5 exact) · V3b EQUIV (pursuer ±15%) · opposition_components (5 traces byte-identical) · pursuing_hazard · run_config R0 · save_migration · encounter_builder inert · smoke · catalog. **Wave-4 close-out: DR-3 SIGNED OFF; all 5 M1.12 deviations dispositioned (4 Reviewed + V4 Addressed via V4b) + archived → `DESIGN_DEVIATIONS_HISTORY.md §M1.12`.** Debt ledger (build phase): index-alignment retired (V1) · 4 interaction copies→1 (V5) · telemetry bounded (V7) · CI trend visible (V8) · dead surface removed (V9) · EventBus 60→54 (V2) · RNG idiom 5 sites→1 surface (V6) · GameState 752→467+251+96 (V4) · **all 3 greybox spawn machines→deck lane, config 91→52, ~−600 hazard-lane LOC (V3+V3b)** — one way to add an opposition.

## ▶ Next action (start here on a cold restart) — **M1.12 is CLOSED (GO → M2).** Nothing in M1.12 is outstanding. The next milestone is **M2** (crafting / upgrades / instability per the TDD roadmap) — it has NOT been opened; when the Director directs it, run the four-phase authoring (Phase 0 folder `design/M2_Tasks/` → Phase 1 breakdown → Phase 2 per-task designs → Phase 3 fresh-eyes resolve → Phase 4 TASKS/board/STATUS wire-up) per CLAUDE.md. The deferred **R1 (CSV item catalog + importer)** is the natural first content iteration (now unblocked by V1's by-id map). **CI/CD stays disabled** (LFS bandwidth) until re-enabled (`gh workflow enable "CI"`/`"nightly-playtest"` + uncomment triggers); the gate meanwhile is local headless verify (grep stderr for `SCRIPT ERROR`, not just OK). Non-blocking older gates still open: M1.11 UG2/UG3, M1.10 TG2/TG3, M1.9 SG2/SG3, M1.7/M1.6 RG2/RG3.
>
> **(archived) prior next-action — M1.12 Wave 5 regression gate** (VG1 verify+changelog no-publish → VG2 → VG3 → GO): complete. **⚠ 2026-07-11 Director directive DONE: ALL CI/CD STOPPED (GitHub LFS bandwidth limit).** Both workflows `gh workflow disable`d (`disabled_manually`) AND their triggers commented to `workflow_dispatch`-only in-file (`a74ca8b`) — no push/PR/nightly runs, no LFS-checkout bandwidth burn. Recent CI/nightly failures were infra (LFS limit), not code (local smoke + full suite green throughout). **Re-enable when bandwidth resets:** `gh workflow enable "CI"` + `gh workflow enable "nightly-playtest"`, then uncomment the trigger blocks in `.github/workflows/*.yml`. **Meanwhile the gate is LOCAL** headless verify (smoke + suite). **VG1 impact:** its itch-publish step (local `butler push` — does NOT use GitHub LFS bandwidth) needs a Director call — still publish the regression build, or run VG1 verify+changelog only? Master contract holds: 4 control layout fps byte-identical + full suite green. Breakdown + D-RAT-1…8: `design/M1_12_Tasks/M1.12_Breakdown.md`; queue: `TASKS.md` §M1.12; board: `design/M1_12_Tasks/.board_item_ids.txt`.
>
