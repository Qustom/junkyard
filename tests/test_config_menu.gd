...
##      count matches the schema (38 knobs: 32 R0 + 1 I2 r1_catch_radius_per_depth + 3 I1 lvl_
##      + 2 J2 r1_spawn_distribution/r1_spread_min_depth).
...
func _run() -> int:
	...
	# Sanity vs. the schema's own count (R0: 32 + I2's 1 r1_ + I1's 3 lvl_ + J2's 2 r1_ = 38).
	if exported.size() != 38:
		failures.append("expected 38 exported RunConfig fields, schema has %d" % exported.size())
