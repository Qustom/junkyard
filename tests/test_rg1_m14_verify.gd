...
func _run() -> int:
	...
	await _drive_run(_default_preset(), &"extract", "M1-default-preset")  # the M1.4 boot stack
...
func _verify_default_preset_shape() -> void:
	...
	# --- K4 timer ON: 60s dive, ~10s warning, visual_only. ---
	if not preset.timer_enabled:
		_failures.append("RG1/K4: default preset timer_enabled is false (the dive timer must be ON)")
	if not is_equal_approx(preset.timer_length_s, 60.0):
		_failures.append("RG1/K4: default preset timer_length_s=%f, expected 60.0" % preset.timer_length_s)
	if not is_equal_approx(preset.timer_warning_threshold_s, 10.0):
		_failures.append("RG1/K4: default preset timer_warning_threshold_s=%f, expected ~10.0"
