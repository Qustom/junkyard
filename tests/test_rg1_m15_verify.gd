...
func _run() -> int:
	...
	if _cfg_menu == null:
		printerr("RG1 M1.5 VERIFY FAIL: ConfigMenu rail not found in assembled scene")
		return 1

	# RG1 (assembled): the CFG rail boots seeded with the default play-preset (M1.5 levers ON).
