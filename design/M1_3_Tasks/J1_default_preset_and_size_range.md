...
- **Preset representation:** static factory `RunConfig.make_default_play_preset()` (Phase-3 A). The code-level all-off `RunConfig.new()` / `run_config.tres` stays the **permanent baseline control** (fp `e943ac9c8bc1`); a single **Reset = all-off** (Phase-3 E). The game/CFG boots into the preset (`config_menu._ready` reseed + `main_game.gd:178` fallback flips to the preset; loop tests stage config explicitly).
- **B (room count): 19** (Director-accepted recommendation; sweepable). **C (size max 40.0): ship `RANGE_MULT = Vector2(4.0, 40.0)`**, step 0.25; determinism/seam-safe (confirmed) — builder records a **manual mult-40 smoke** (camera reads, no seam, run completes) in the worklog; surface a lower cap only if unplayable.
- **D (preset magnitudes):** lift the R1/R4 values from the M1.2 most-fun cell in the run log; the one new value is **`r4_lost_proxy_threshold ≈ 0.5`** (M1.2 ran it at the config-trap 0.0) — pairs with BUG6's guard.
- **Wave-1 ownership:** **BUG6's `inert_enabled_oppositions()` lands first; J1 rebases its preset factory on top** (and may assert the preset is trap-free). J1 owns `config_menu.gd` (incl. folding BUG6's CFG warn-line) + the `run_config.gd` knob-add pass that **pre-declares J2/J3's `r1_*` spawn knobs**. No EventBus pre-declare needed in Wave 1.
