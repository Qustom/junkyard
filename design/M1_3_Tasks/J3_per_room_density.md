...
- **Density model (the load-bearing call): Director chose CELL-AREA** (size-invariant) — hazard count keyed to `floor_cells.size()` so a 40× room holds a fixed number of threats per room-shape (bounded, perf-safe). **px-area is built as a swept option** (capped). New `r1_per_room_density` + supporting knobs (min-area floor, `r1_density_rooms_only`, per-room cap mandatory in the preset).
- **B (fill with what): hazards primary; loot-per-area built but OFF by default** (swept lever; never preset-on — it contradicts `depth_curve.gd`'s "don't flood deep rooms"; RG2 segments on it).
- **Build structure:** **J2 first, then J3 additive** on `main`, single owner of `_spawn_r1_hazards`/`_hazard_spawn_position`; J3 = the per-room **budget**, J2 = the **distribution**; J1 pre-declares the knobs. The optional loot sub-knob touches `junk_placer.gd` (disjoint single-writer).
