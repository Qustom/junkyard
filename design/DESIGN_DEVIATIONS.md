...
### C2 — Junk pickup in the band (integrated `2026-06-17`, impl `5adacac`)
- `[2026-06-17] C2/seam` — `JunkSpawner` is a **pure consumer of B3's `JunkPlacer.plan()`**: `populate(plan, container)`, no own item-weighting and no `RNG.stream("junk_spawn")` (that API doesn't exist; B3 owns all deterministic placement incl. per-anchor density). · Orchestrator-decided B3↔C2 seam; one deterministic placement authority. · **Rec: Reviewed (record the seam in `M1_As_Built.md`).**
- `[2026-06-17] C2/trigger` — Spawner is invoked **directly after generation** (sequenced sub-step), not via a fire-and-forget EventBus signal. · Per the spec's own recommendation (hard ordering + data dependency). · **Rec: Reviewed.**
- `[2026-06-17] C2/signals` — Added `junk_picked_up(item_id, value, slot_size, world_pos, accepted)` (primitives-only, value snapshotted, includes `slot_size` for capacity analytics), plus `band_populated(count)` and `junk_dropped(item, world_pos)`. · Per the spec recommendation; payload extended for telemetry. · **Rec: Reviewed.** *(Handoff: confirm `junk_picked_up`/`band_populated` are in Telemetry's watch list.)*
- `[2026-06-17] C2/schema+ux` — Consumed `JunkItem.base_sell_value` (item arrives as B3's depth-scaled `duplicate(true)`); reject-UX keys off the same D1 `can_accept()`/`is_full()` truth D2's panel reads (pickup flashes red + stays in world on full bag). · C1b + cross-surface consistency with D2. · **Rec: Reviewed.**
- `[2026-06-17] C2/test` — C2 acceptance test runs as a `.tscn` (`godot --headless res://tests/test_junk_pickup.tscn`) because it needs the `EventBus`/`RNG`/`GameState` autoloads. · Same harness constraint as B3. · **Rec: Reviewed (fold the autoload-test run command into `M1_As_Built.md`).**
- `[2026-06-17] C2/dropwiring` — **Follow-up, NOT done in C2 (D2 already merged; its files left untouched):** the drop-to-swap re-spawn path is wired on the spawner side (`spawn_one()` + `junk_dropped` subscription) but stays **dormant** until D2's drop gesture emits `EventBus.junk_dropped.emit(removed_item, drop_world_pos)` after `RunInventory.remove_at()` (a one-line D2 addition). · The seam is built; D2 doesn't yet emit. · **Rec: Addressed — plan a small task/touch to add the D2 emit so drop-to-swap is live (`ui-ux-designer`).**
