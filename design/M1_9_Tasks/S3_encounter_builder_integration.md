...
*Spec authored for M1.9 S3 (Phase-2 fan-out). Phase 3 assigns a fresh-eyes resolver to §6; anything vision/fun/scope-flavored there (notably Q1's acceptance bar and Q4's scope call if the resolver disagrees with the recommendation) goes to the Director with a recommendation per the orchestrator loop. Deviations during the build go to `design/DESIGN_DEVIATIONS.md` for the Wave-3 close-out sweep.*
