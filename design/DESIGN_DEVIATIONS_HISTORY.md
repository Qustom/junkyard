...
assessment (`CLAUDE.md` → "Wave close-out — deviation assessment"). Each entry is tagged:

- **Reviewed** — fine as-is; the design needed no change.
- **Addressed** — the design was changed; the "Reapplied to" column says where the now-canonical
...
## M1 waves 1 & 2 — assessed 2026-06-17

All entries below were verified on `main` (waves 1 & 2 integrated) and resolved. No deviation was a
revert; every `Addressed` item was reapplied. The two recurring themes — the specs' **idealized
API sketches** and the **headless-test autoload constraint** — are now canonicalized in
`design/M1_Tasks/M1_As_Built.md`.

| # | Deviation | Disposition | Reapplied to |
|---|---|---|---|
...
| W1-4 | C1 `engine_block.slot_size = 6` (spec sketch showed 4) | **Addressed** | `M1_As_Built.md` §Tuning dials (flagged for economy workbook / G4) |
| W2-5 | A3 dive clock keys off existing `run_started`/`run_ended` — no new `dive_started`/`dive_ended` | **Addressed** | `M1_As_Built.md` §EventBus dive lifecycle contract |
...
| W2-8 | Open follow-up: `Item` vs `JunkItem` schema overlap | **Addressed** | Resolved by Decision #1 (merge); planned as task **C1b** (in progress) |
| W2-9 | Open follow-up: parallel-dispatch `git switch` collisions in a shared checkout | **Addressed** | Process rule in `CLAUDE.md` (worktree isolation + pre-lock EventBus signals) |
| W2-10 | B2 RNG API adaptation (`seed_from` + self-written integer weighted pick; spec's `set_seed`/`weighted_pick`/`fork` don't exist) | **Addressed** | `M1_As_Built.md` §RNG |
...
| W2-17 | D1 added index-safe `remove_at(index)` alongside `remove(item)` | **Reviewed** | — (adopts the D1 spec's own Open-question recommendation) |

**New tasks planned from this assessment:** none beyond what already exists — W2-8 became **C1b** (already on the board, in progress); W2-9 became the standing worktree process rule; W2-13/16's "revisit" is already scoped under **G2** (vendor GdUnit4).
